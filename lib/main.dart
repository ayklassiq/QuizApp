import 'package:firebase_core/firebase_core.dart';
import 'package:flutter/material.dart';
import 'package:provider/provider.dart';
import 'package:quiz_app/service/database.dart';
import 'package:quiz_app/view/createquizpage.dart';
import 'package:quiz_app/view/homepage.dart';
import 'package:quiz_app/view/leadership.dart';
import 'package:quiz_app/view/quizpage.dart';
import 'package:quiz_app/view/userCreateQuizPage.dart';
import 'package:quiz_app/viewmodel/quizviewmodel.dart';
import 'package:quiz_app/viewmodel/useraddquizviewmodel.dart';

Future main() async  {

  WidgetsFlutterBinding.ensureInitialized();
  await Firebase.initializeApp();
  await MongoDatabase.connect();
  runApp(const MyApp());
}

class MyApp extends StatelessWidget {
  const MyApp({super.key});

  // This widget is the root of your application.
  @override
  Widget build(BuildContext context) {
    return MultiProvider(
      providers: [
        ChangeNotifierProvider(
          create: (ctx) =>QuizViewModel(),
        ),
        ChangeNotifierProvider(
          create: (ctx) =>UserQuizViewModel(),
        ),
      ],      child: MaterialApp(
        title: 'Flutter Demo',
        theme: ThemeData(
          // This is the theme of your application.
          //
          // Try running your application with "flutter run". You'll see the
          // application has a blue toolbar. Then, without quitting the app, try
          // changing the primarySwatch below to Colors.green and then invoke
          // "hot reload" (press "r" in the console where you ran "flutter run",
          // or simply save your changes to "hot reload" in a Flutter IDE).
          // Notice that the counter didn't reset back to zero; the application
          // is not restarted.
          primarySwatch: Colors.blue,
        ),
          home: HomeScreen(),
    routes: {

      QuizScreen.routeName: (ctx) => QuizScreen(),
      CreateQuizScreen.routeName:(ctx)=>CreateQuizScreen(),
      HomeScreen.routeName:(ctx)=>HomeScreen(),
       UserQuizScreen.routeName:(ctx)=>UserQuizScreen(),
      LeaderboardScreen.routeName:(ctx)=>LeaderboardScreen()

    }));}}